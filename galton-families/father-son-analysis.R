...
ggsave("fig/scatterplot-father-son-correlation.png")

# Correlation of father, son heights
galton_heights <- GaltonFamilies %>% 
...
GaltonFamilies %>% filter(father==72)
...
m <- r * s_y/s_x
b <- mu_y - m*mu_x
# add regression line to plo-t (son given father)
galton_heights %>%
  ggplot(aes(father, son)) +
  geom_point(alpha = 0.5) +
...
m_2 <-  r * s_x / s_y
b_2 <- mu_x - m_2*mu_y
# add regression line to plot (father given son)
galton_heights %>%
...
# Because not all sons with fathers of height x are of equal height, we need the term epsilon, which explains the remaining variability.
...
# LSE are random variables since they are derived from the samples
...
#  standardize the father heights
B <- 1000
N <- 50
lse <- replicate(B, {
  sample_n(galton_heights, N, replace = TRUE) %>%
